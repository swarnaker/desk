...
import { RadarTable } from "@/components/Lane";
...
export default function FirstPage() {
  ...
  const firstFilters = { ...filters, firstOnly: true };
  ...
  const filtered = useMemo(
    () => applyFilters(tokens, firstFilters, watch.ids),
    [tokens, firstFilters, watch.ids],
  );
    ...
    <div className="space-y-3">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <span className="chip chip-on tracking-[0.2em]">FIRST HOUR</span>
          <span className="text-[11px] text-mute">Tokens marked for first-hour tracking</span>
        </div>
      </div>
      <FilterBar
      ...
      {!isLoading && filtered.length === 0 ? (
        <div className="border border-hairline bg-surface px-4 py-10 text-center text-[12px] text-mute">
          {data?.stale ? "STALE · empty board. Adapters missed or Dex is down." : "No FIRST tokens. Mark tokens as FIRST from the token desk to track them here."}
        </div>
      ) : null}
      ...
      {filtered.length > 0 ? (
        <RadarTable
          ...
          onWatch={(r) => watch.toggle(r.chain, r.ca)}
          sortColumn={sortColumn}
          sortDirection={sortDirection}
